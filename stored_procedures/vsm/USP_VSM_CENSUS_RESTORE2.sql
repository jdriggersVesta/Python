CREATE OR REPLACE PROCEDURE USP_VSM_CENSUS_RESTORE2()
  RETURNS VARCHAR(10)
  LANGUAGE javascript
  AS
  $$
...
var cmd=`TRUNCATE VESTA.VESTA_MEMBERCENSUS;`;
  ...
  sql.execute();
    ...
    /*********************
         VESTA TABLE         
    *********************/
/*    var cmd=`INSERT INTO VESTA.VESTA_MEMBERCENSUS_NEW */
...
//Following fields are no longer needed here. They are inserted into Census table from VESTA.V_PROGRAMCONFIGMODEL later
//                ,o.client_name
//                ,o.client_abbr
//                ,o.program_name
//                ,o.program_abbr
...
//Following section is replaced in Census table insert by join to VESTA.V_PROGRAMCONFIGMODEL
//                LEFT JOIN
//                (
//                  SELECT
...
//                      ,o.program_configid
//                      ,pcm.EFFECTIVE_DATE
//                      ,CASE WHEN pcm.DISCONTINUE_DATE IS NULL THEN DATE(GETDATE()) END AS DISCONTINUE_DATE
//                  FROM 
...
//                )o ON o.dash_id = p.id AND x.member_censusdatechange BETWEEN o.EFFECTIVE_DATE AND o.DISCONTINUE_DATE
                LEFT JOIN
                (
...
/*   var cmd=`TRUNCATE VSM.CENSUS_NEW;`; */
   var cmd=`TRUNCATE VSM.CENSUS;`;
   var sql=snowflake.createStatement({sqlText: cmd});
   sql.execute();

/*   var cmd=`INSERT INTO VSM.CENSUS_NEW */
   var cmd=`INSERT INTO VSM.CENSUS
          ...
          clients.PROGRAM_CONFIGURATION_ID AS PROGRAM_CONFIGID,
 //    clients.PCM_EFFECTIVE_DATE,                              --NEW FIELD
 //    clients.PCM_DISCONTINUE_DATE,                            --NEW FIELD
      ...
      LEFT JOIN
        (
          SELECT 
            pcm.DASH_ID,
            pcm.PROGRAM_CONFIGURATION_ID,
            pcm.PCM_EFFECTIVE_DATE,
            CASE WHEN pcm.PCM_DISCONTINUE_DATE IS NULL THEN DATE(GETDATE()) ELSE pcm.PCM_DISCONTINUE_DATE END AS PCM_DISCONTINUE_DATE,
            vc.CLIENT_NAME,
            vc.CLIENT_ABBR,
            vc.PROGRAM_NAME,
            vc.PROGRAM_ABBR
          FROM
             VESTA_DEVELOPMENT.VESTA.V_PROGRAMCONFIGMODEL pcm
          LEFT JOIN
            VESTA.V_VESTACLIENTS vc ON pcm.PROGRAM_CONFIGURATION_ID = vc.PROGRAM_CONFIGID --WHERE PCM.DASH_ID = 2235
         ) clients ON dates.CALENDAR_DATE BETWEEN clients.PCM_EFFECTIVE_DATE AND clients.PCM_DISCONTINUE_DATE --WHERE clients.DASH_ID = 2235 AND YEAR(dates.CALENDAR_DATE) > 2018 ORDER BY CALENDAR_DATE DESC
      LEFT JOIN
        (
          SELECT 
            *,
...
/*          FROM VESTA.VESTA_MEMBERCENSUS_NEW c */
          FROM VESTA.VESTA_MEMBERCENSUS c
        )census ON dates.CALENDAR_DATE BETWEEN census.member_statusstartdate AND census.FIXDATE 
              --AND census.PROGRAM_CONFIGURATION_ID = clients.PROGRAM_CONFIGURATION_ID
              AND clients.DASH_ID = census.DASH_ID
      LEFT JOIN
